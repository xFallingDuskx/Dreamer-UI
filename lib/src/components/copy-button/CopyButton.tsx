...
import { Check, Copy } from '../../symbols';
...
export interface CopyButtonProps {
	...
	/** Custom size for the icons (default: 16). */
	iconSize?: number
...
export function CopyButton({
	...
	return (
		...
		>
			{copied ? (
				<>
					{copiedIcon ?? <Check size={iconSize} className={iconClassName} />}
					{children || (showCopyText && 'Copied!')}
				</>
			) : (
				<>
					{icon ?? <Copy size={iconSize} className={iconClassName} />}
