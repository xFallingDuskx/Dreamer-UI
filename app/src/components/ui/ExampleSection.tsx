...
function SectionHeader({ id, children, level = 3, className }: SectionHeaderProps) {
	...
	const textSize = level === 1 ? 'text-3xl' : level === 2 ? 'text-2xl' : 'text-xl';

	return (
		<Tag
			className={join('group relative flex items-center font-semibold text-white mb-2', textSize, className)}
...
export function ExampleSection({ title, description, children, className = '', id, code }: ExampleSectionProps) {
	...
	return (
		<section className={`mb-12 ${className}`} id={sectionId}>
			<div className='mb-6'>
				<SectionHeader id={sectionId} level={3}>
					{title}
				</SectionHeader>
